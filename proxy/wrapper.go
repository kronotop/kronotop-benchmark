...
func (c *ConnWrapper) Read(b []byte) (n int, err error) {
	n, err = c.Conn.Read(b)
	log.Info().Int64("conn_id", c.connId).Str("label", c.label).Int("read_bytes", n).Msg(string(b[:n]))
	return n, err
}

func (c *ConnWrapper) Write(b []byte) (n int, err error) {
	n, err = c.Conn.Write(b)
	log.Info().Int64("conn_id", c.connId).Str("label", c.label).Int("write_bytes", n).Msg(string(b[:n]))
	return n, err
}
